...
def create_heatmap_plot(json_file_path, output_png_path, sigma=15):
    ...
    try:
        ...
        print(
            f"Error: Could not decode JSON from '{json_file_path}'. Is it a valid JSON file?"
        )
        return

    if (
        not heatmap_data
        or not isinstance(heatmap_data, list)
        or not all(isinstance(row, list) for row in heatmap_data)
    ):
        print("Error: Invalid heatmap data format. Expected a list of lists.")
        print(
            "Please ensure the JSON file contains a 2D array (e.g., [[0, 100, 0], [0, 0, 200]])."
        )
        return
    ...
    smoothed_heatmap = gaussian_filter(
        np_heatmap, sigma=sigma, mode="constant", cval=0.0
    )
    ...
    if max_val == 0:
        print(
            "Warning: Heatmap contains only zero values after smoothing (or no Waldo finds were recorded)."
        )
        print(
            "No meaningful plot can be generated. Please ensure your JSON has non-zero time values."
        )
        return
    ...
    base_width = 12
    base_height = (
        base_width * (height / width) if width > 0 else base_width * (3 / 4)
    )  # Default to 4:3 if width is 0
    plt.figure(figsize=(base_width, base_height))

    # Use 'binary' colormap: white for low values (fast times), progressing through black for high values (slow times).
    ...
    plt.title(
        f"Waldo Field of Vision Heatmap (Smoothed with $\sigma={sigma}$)"
    )
